/**
 * boomboxSystem.ts — Click boombox to toggle music mute/unmute.
...
const RING_INTERVAL = 0.8
const RING_COLOR = Color4.create(1, 0.84, 0, 0.25)
...
// Ring pool
const RING_POOL_SIZE = RING_COUNT + 1
interface Ring {
  entity: Entity
  age: number
  active: boolean
...
export function boomboxSystem(dt: number): void {
  ...
  for (const ring of ringPool) {
    ...
    const alphaStep = Math.floor(progress * 5)
