...
import { Sword, SwordState, CountdownTimer } from '../shared/components'
import { room } from '../shared/messages'
// Shield system removed for Contagion
// Lightning state removed for Contagion
...
const SWORD_MODEL_SRC = 'assets/models/sword.glb'
...
function initClonePool(): void {
  ...
  console.log('[Sword] Clone pool initialized (Anchor → StaticOffset → Visual)')
}
...
// State tracking
let prevSwordState: SwordState | null = null
...
room.onMessage('swordPickupConfirmed', (data) => {
...
let swordVisualEntity: Entity | null = null
export let swordSyncedEntity: Entity | null = null
let swordBobTime = 0
let swordModelAttached = false

function ensureFlagModel(): void {
  if (swordModelAttached) return
  for (const [entity] of engine.getEntitiesWith(Sword, Transform)) {
    // Create a local child entity for the visual model + bob/spin
    swordVisualEntity = engine.addEntity()
    Transform.create(swordVisualEntity, {
      parent: entity,
      position: Vector3.create(0, 0, 0)
    })
    GltfContainer.create(swordVisualEntity, {
      src: SWORD_MODEL_SRC,
      visibleMeshesCollisionMask: 0,
      invisibleMeshesCollisionMask: 0
    })
    swordSyncedEntity = entity
    swordModelAttached = true
    console.log('[Sword] 🚩 Created local visual child with bob/spin')
    break
  }
}
...
function updateFlagBob(dt: number): void {
  swordBobTime += dt
  const bobY = IDLE_BOB_AMPLITUDE * Math.sin(swordBobTime * IDLE_BOB_SPEED)
  const angleDeg = (swordBobTime * IDLE_ROT_SPEED_DEG_PER_SEC) % 360

  // Animate the idle flag visual (at base or dropped)
  if (swordVisualEntity && Transform.has(swordVisualEntity)) {
    const flag = swordSyncedEntity ? Sword.getOrNull(swordSyncedEntity) : null
    if (flag && flag.state !== SwordState.Carried) {
      const t = Transform.getMutable(swordVisualEntity)
...
export function swordClientSystem(dt: number): void {
  ...
  if (userId) {
    for (const [, flag] of engine.getEntitiesWith(Sword)) {
      if (flag.state !== SwordState.Carried && prevSwordState === SwordState.Carried && prevCarrierId === userId) {
  ...
  if (userId && Transform.has(engine.PlayerEntity)) {
    ...
    for (const [, flag] of engine.getEntitiesWith(Sword)) {
      if (flag.state === SwordState.Carried && flag.carrierPlayerId === userId) {
        amCarrying = true
        break
      }
    }

    if (!amCarrying && !false && now - lastAutoPickupRequestMs >= AUTO_PICKUP_COOLDOWN_MS && now - lastDropTimeMs >= DROP_PICKUP_COOLDOWN_MS && now - lastWitnessedDropTimeMs >= WITNESSED_DROP_COOLDOWN_MS) {
      const myPos = Transform.get(engine.PlayerEntity).position
      for (const [flagEnt, flag] of engine.getEntitiesWith(Sword, Transform)) {
        if (flag.state === SwordState.Carried) continue
        ...
        if (dist <= AUTO_PICKUP_RADIUS) {
          ...
          if (swordVisualEntity) VisibilityComponent.createOrReplace(swordVisualEntity, { visible: false })
          showClone(userId)
          playPickupSound()
          ...
          room.send('requestSwordPickup', { t: 0 })
  ...
  // ── Sword attack (E key) — only sword carrier can attack nearby slimes ──
  if (inputSystem.isTriggered(InputAction.IA_PRIMARY, PointerEventType.PET_DOWN) && userId) {
    let amCarrying = false
    for (const [, sword] of engine.getEntitiesWith(Sword)) {
      if (sword.state === SwordState.Carried && sword.carrierPlayerId === userId) {
        amCarrying = true
        break
      }
    }
    ...
    if (amCarrying) {
      room.send('requestSwordAttack', { t: 0 })
    }
  }
  ...
  if (pendingPickupUntil > 0 && Date.now() > pendingPickupUntil) {
    console.log('[Sword] ⏪ Pickup not confirmed — rolling back')
    pendingPickupUntil = 0
    hideClone()
    // shield removed
    if (swordVisualEntity) VisibilityComponent.createOrReplace(swordVisualEntity, { visible: true })
  ...
  for (const [, flag] of engine.getEntitiesWith(Sword)) {
    const stateChanged = prevSwordState !== null && prevSwordState !== flag.state
    const carrierChanged = flag.state === SwordState.Carried && flag.carrierPlayerId !== prevCarrierId && prevCarrierId !== ''

    // Determine what changed
    const isFirstFrame = prevSwordState === null
    const needsCloneCreate = flag.state === SwordState.Carried && (
    ...
    const needsCloneRemove = flag.state !== SwordState.Carried && (
      isFirstFrame ||
      stateChanged
    )
    ...
    if (needsCloneCreate) {
      ...
      if (flag.state === SwordState.Dropped) {
    ...
    // 1. Sword IS carried — ensure clone is showing for the correct carrier
    if (flag.state === SwordState.Carried && !needsCloneCreate) {
      const wrongOrMissing = !cloneVisible || carryCloneCarrierId !== flag.carrierPlayerId
      if (wrongOrMissing) {
        console.log(`[Sword] ⚠️ Safety net: showing clone for correct carrier`)
        if (swordVisualEntity) VisibilityComponent.createOrReplace(swordVisualEntity, { visible: false })
        showClone(flag.carrierPlayerId)
      }
    }
    
    // 2. Sword is NOT carried — hide any lingering clone + restore flag visual
    ...
    if (flag.state !== SwordState.Carried && pendingPickupUntil === 0 && !inGracePeriod) {
      if (cloneVisible) {
        console.log('[Sword] ⚠️ Safety net: hiding orphaned clone (flag not carried)')
        hideClone()
        // (no-op)()
      }
      if (swordVisualEntity) {
        if (!VisibilityComponent.has(swordVisualEntity)) {
          VisibilityComponent.create(swordVisualEntity, { visible: true })
        } else if (!VisibilityComponent.get(swordVisualEntity).visible) {
          VisibilityComponent.createOrReplace(swordVisualEntity, { visible: true })
        }
      }
    }

    prevSwordState = flag.state
  ...
  for (const [flagEntity, flag] of engine.getEntitiesWith(Sword, Transform)) {
    if (flag.state === SwordState.Carried && flag.carrierPlayerId) {
      beaconSpawnAccum = 0 // No beacon particles when carried
      
    } else if (flag.state === SwordState.AtBase || flag.state === SwordState.Dropped) {
