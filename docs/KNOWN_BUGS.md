...
- **Duplicate avatar entities poisoned position history.** `recordPlayerPositions` sampled per ENTITY into a per-ADDRESS history while `getPlayerPosition` picked the newest entity, so with a duplicate present a stale corpse entity's positions were interleaved with the live player's under one key — and `wasWithinRadius` accepts if ANY sample matches. A frozen corpse could therefore keep authorizing projectile hits (`combat.ts` shell path) and client action positions (`resolveActionPosition`) for the live player: the phantom-combat symptom, produced by the scene's own bookkeeping rather than the platform. Both readers now resolve through the same `selectNewestPerAddress` (`src/server/identitySweep.ts`, unit-tested), so they cannot drift.
  ...
  **Caveat added 2026-08-17, after the resolution rule was audited: the rule itself is unsound, and it is kept that way on purpose.** "Highest entity id == newest" is false — the version occupies the high 16 bits and counts how many times that SLOT was recycled, not when its occupant was assigned. Since the runtime hands a reconnecting player whichever slot is free, an address can move to a *lower* raw id: **37 of 103 consecutive reallocations did so in the 2026-08-15 log** (e.g. `#35 v21` = 1376291 → `#37 v8` = 524325), and in each of those the resolution returns the corpse. It is retained because it is wrong *symmetrically* — the answer is a pure function of the entity set, so the server and every client agree, and a beacon pointing where hit detection is not would be harder to diagnose than both being wrong together. A first-sighting recency fix was written, audited, and rejected for introducing exactly that divergence, and because it degrades to this same rule for any duplicate that predates the observer (the common case on a client). The duplicate condition needs a lost `DELETE_ENTITY` tombstone, which the runtime bounds at 4096+ departures during a single VM stall; `👥 duplicate PlayerIdentityData` fired **zero** times in 3 h. If it ever fires in the field, the fix is an observed-liveness signal applied at *every* address→entity lookup — there are ~12, and nine still take first match by iteration order (`projectile/flight.ts`, `boostTrailSystem.ts`, `remoteBoomerangSystem.ts`, `lightningSystem.ts`, `spectatorSystem.ts`, `combatSystem.ts`, `roundManager.ts`, `playerTracking.ts`) — not by changing one comparison.
